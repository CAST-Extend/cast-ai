# CAST Copilot Toolset

OSS vulnerability assessment and remediation using CAST Highlight,
powered by GitHub Copilot agent in VS Code.

**Enter the app name once in Step 1. Steps 2 and 3 pick up automatically from the same chat session.**
...
│   └── workspace-dependency-analysis.md ← workspace blast radius lookup for one OSS component
└── prompts/
    ├── app-resolver.prompt       ← Step 1: resolve app ID once per session
...
> | `.vscode/mcp.json` | **Add** the `cast-highlight` server entry to your existing JSON object |
...
### 2. Set your Highlight MCP URL in mcp.json

```
CAST_HIGHLIGHT_MCP_URL=https://your-highlight-mcp-server/mcp
```

### 3. Connect MCP servers in VS Code

Open Copilot Chat → switch to **Agent** mode → click the tools icon → confirm
`cast-highlight` is listed. You will be prompted for your API key on first use each session.
...
Use APP_NAME, HIGHLIGHT_NAME, and HIGHLIGHT_APP_ID
...
- Confirm oss_remediation_plan.md exists and contains a P1 section.
If either fails: stop and report — do not proceed.

Per P1 item (one at a time, P1.1 then P1.2 etc.):
1. Run workspace-dependency-analysis skill:
   Search pom.xml / package.json / build.gradle / *.csproj for component declaration.
   Search source files for imports and call sites.
   Identify APIs used. Assess break risk.
2. Break risk found — note, propose resolution, confirm before proceeding.
3. Apply fix (nearest safe). Use latest safe only if nearest breaks a dependency.
4. Wrap changes:
   // AI remediation begin — <component> <old_version> to <new_version>
   // AI remediation end
5. Run build. Pass — next. Fail — revert, log reason, skip.
6. Output completion summary table after all P1 items.
