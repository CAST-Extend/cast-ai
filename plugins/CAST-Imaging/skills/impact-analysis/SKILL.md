...
1. Add the CAST Imaging MCP server to your Claude Code settings
...
- **Multiple matches** — **HARD STOP.** Present a numbered list of ALL matching objects and ask the user to select one before proceeding. Never auto-select. Format the list as:
  ...
  ```
  **Do NOT proceed to Step 3 until the user has explicitly selected an object.** This applies even if the matches look related (e.g., an object and its child fragment) — always let the user decide.
...
CHILD_SCORE       = child_count          # children are lost if the parent is removed
...
If the user asks to persist findings, tag all impacted objects (target, callers, and callees) in a single call:
...
This allows filtering by tag in CAST Imaging to instantly visualize the entire blast radius as a graph.
...
6. **Paginate** — if any tool returns `has_next: true`, fetch subsequent pages. For steps with >20 results, show the top 20 in tables and summarize the rest with a count.
7. **Include all output sections** even if empty ("None detected").
8. **Calculate risk score** using the formula — never eyeball it.
9. **Handle errors gracefully** — if a tool call fails, report the error in the corresponding output section and continue with the remaining steps.
